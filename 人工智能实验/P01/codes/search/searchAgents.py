...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (1, 1, 1, 1, self.startingPosition)
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        if state[0] is 0 and state[1] is 0 and state[2] is 0 and state[3] is 0:
            return True
        return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            dx, dy = Actions.directionToVector(action)
            x, y = int(state[4][0] + dx), int(state[4][1] + dy)
            if 0 <= x and x < self.walls.width and 0 <= y and y < self.walls.height and not self.walls[x][y]:
                state_list = list(state)
                for i in range(4):
                    if (x, y) == self.corners[i]:
                        state_list[i] = 0
                state_list[4] = (x, y)
                successors.append((tuple(state_list), action, 1))
        self._expanded += 1 # DO NOT CHANGE
        return successors
