...
def naiveSoftmaxLossAndGradient(
    ...
    p = softmax(outsideVectors @ centerWordVec)  # (num words in vocab, )
    loss = -np.log(p[outsideWordIdx])  # (1, )
    p[outsideWordIdx] -= 1
    gradCenterVec = p @ outsideVectors # (word vector length, )
    gradOutsideVecs = p.reshape(-1, 1) @ centerWordVec.reshape(1, -1)  # (num words in vocab, word vector length)
...
def negSamplingLossAndGradient(
    ...
    p_t = sigmoid(outsideWordVec @ centerWordVec)
    p_f = sigmoid(-negSampleWordVecs @ centerWordVec)
    loss = -np.log(p_t) - np.sum(np.log(p_f))
    gradCenterVec = (p_t - 1) * outsideWordVec + (1 - p_f) @ negSampleWordVecs
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx, :] = (p_t - 1) * centerWordVec
    for i in range(len(negSampleWordIndices)):
        gradOutsideVecs[negSampleWordIndices[i]] += (1 - p_f[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        loss_temp, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec, word2Ind[outsideWord], outsideVectors, dataset)
        loss += loss_temp
