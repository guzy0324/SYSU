if __name__ == '__main__':
    ...
    if args.function == 'pretrain':
        ...
        config = trainer.TrainerConfig(max_epochs=650,
                                       batch_size=128,
                                       learning_rate=6e-3,
                                       lr_decay=True,
                                       warmup_tokens=512 * 20,
                                       final_tokens=200 * len(pretrain_dataset) * block_size,
                                       num_workers=4)
        trainer = trainer.Trainer(model, pretrain_dataset, None, config)
        trainer.train()
        ...
        if args.reading_params_path:
            ...
            config = trainer.TrainerConfig(max_epochs=10,
                                           batch_size=256,
                                           learning_rate=6e-4,
                                           lr_decay=True,
                                           warmup_tokens=512 * 20,
            ...
            config = trainer.TrainerConfig(max_epochs=75,
                                           batch_size=256,
                                           learning_rate=6e-4,
                                           lr_decay=True,
                                           warmup_tokens=512 * 20,
                                           final_tokens=200 * len(pretrain_dataset) * block_size,
                                           num_workers=4)
        trainer = trainer.Trainer(model, finetune_dataset, None, config)
        trainer.train()
        torch.save(model.state_dict(), args.writing_params_path)
