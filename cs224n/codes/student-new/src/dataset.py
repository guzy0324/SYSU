...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_document = document[:random.randint(4, int(self.block_size * 7 / 8))] + (4 - len(document)) * self.PAD_CHAR
        start = random.randint(1, len(truncated_document) - 2)
        end = random.randint(start + 1, len(truncated_document) - 1)
        prefix = truncated_document[:start]
        masked_content = truncated_document[start:end]
        suffix = truncated_document[end:]
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        masked_string += (self.block_size - len(masked_string)) * self.PAD_CHAR
        input_string = masked_string[:-1]
        output_string = masked_string[1:]
        x = torch.tensor(list(map(self.stoi.get, input_string)), dtype=torch.long)
        y = torch.tensor(list(map(self.stoi.get, output_string)), dtype=torch.long)
        return x, y
"""
